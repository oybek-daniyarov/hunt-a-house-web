...
import { LEAD_FILTER_MAPPINGS, LEAD_SORT_MAPPINGS, type LeadFilterParams } from "./lead.types"
...
export function buildLeadFilters(params: LeadFilterParams): Filter[] {
  ...
  Object.entries(params).forEach(([key, value]) => {
    ...
    const fieldName = LEAD_FILTER_MAPPINGS[key as keyof typeof LEAD_FILTER_MAPPINGS]
    if (!fieldName) return
...
export function buildLeadSorting(sort?: string): SortOption[] {
  ...
  const sortConfig = LEAD_SORT_MAPPINGS[sort as keyof typeof LEAD_SORT_MAPPINGS]
