...
const getEmailSubject = (listing: App.Data.Lead.LeadListResponse) => {
    return encodeURIComponent(`${listing.property_type_name} in ${listing.emirate_name} - Inquiry`)
}
...
const contactMethods = [
    ...
    },
    {
        icon: MessageSquare,
        label: "WhatsApp",
        href: (contact: ContactMethodsProps['contact'], listing: App.Data.Lead.LeadListResponse) => 
    ...
    return (
        <div className="space-y-3 rounded-lg bg-gradient-to-br from-background to-muted/50 p-4">
            ...
            <div className="grid gap-2">
                {contactMethods.map(({ icon: Icon, label, href, color, bgColor, external }) => (
                    <Button
                        key={label}
                        asChild
                        variant="outline"
                    ...
                    >
                        <a
                            href={typeof href === 'function' ? href(contact, listing) : href}
