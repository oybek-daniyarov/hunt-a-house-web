...
type FilterKey = 'emirates' | 'cities' | 'areas' | 'property_types' | 'activity_types' | 'bedrooms' | 'bathrooms'
...
export function PropertyListingSelections({filterData}: Props) {
    ...
    const getLabel = (type: FilterKey, value: string) => {
        ...
        return typeData.find((filter) => filter.id === parseInt(value))?.name ?? value
    }

    const singleFilters = [
        {key: 'search', label: `Search: ${filters.search}`, show: filters.search},
        {key: 'emirate', label: getLabel("emirates", filters.emirate), show: filters.emirate},
        {key: 'propertyType', label: getLabel("property_types", filters.propertyType), show: filters.propertyType},
        {key: 'activityType', label: getLabel("activity_types", filters.activityType), show: filters.activityType},
        {key: 'bedrooms', label: getLabel("bedrooms", filters.bedrooms), show: filters.bedrooms},
        {key: 'bathrooms', label: getLabel("bathrooms", filters.bathrooms), show: filters.bathrooms},
    ]
    ...
    return (
        <div className="container py-2 flex flex-wrap gap-2">
            {singleFilters.map(({key, label, show}) => show && (
                <FilterBadge
                    key={key}
                    label={label}
            ...
            {rangeFilters.map(({show, label, minKey, maxKey}) => show && (
                <FilterBadge
                    key={minKey}
                    label={label}
                    onRemove={() => removeRangeFilter(minKey as keyof typeof filters, maxKey as keyof typeof filters)}
                />
            ))}
        </div>
    )
